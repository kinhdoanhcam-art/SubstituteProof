...
/**
 * Put the wallet on the configured chain before anything is signed.
 *
 * genlayer-js skips its own chain assertion for Studio chains (`isStudio`), so a
 * wallet left on another network would otherwise sign against the wrong chain id
 * with no error surfaced. This is that missing check.
 *
 * Note the limit: Studio Next and Studio Dev share chain id 61997, so a matching
 * id does not by itself prove the wallet's RPC points at the same host.
 */
export async function ensureNetwork(): Promise<void> {
  ...
  try {
    ...
    if (code === 4902) {
      ...
      throw new Error(
        'You declined the network switch. SubstituteProof can only write on ' +
  ...
  if (typeof after !== 'string' || parseInt(after, 16) !== GENLAYER_CHAIN_ID) {
    throw new Error(
      'Wallet is not on ' + GENLAYER_NETWORK.chainName + ' (chain id ' + GENLAYER_CHAIN_ID + ').',
    );
  }
}

/** Connect and return the account address only — the fee kit is built from it. */
 ...
 * `useTxGate()`, which runs the Consensus v0.6 estimate → fee review → sign →
 * track flow and resolves once the transaction is decided. Every call site keeps
 * its own contract-state postcondition check afterwards — a decided transaction
